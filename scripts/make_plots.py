...
OUTDIR = '/home/apurba/sph-hvi-impact-simulation/outputs'
...
r1 = pickle.load(open('/home/apurba/sph-hvi-impact-simulation/results/AlAl_dx1mm.pkl', 'rb'))
r2 = pickle.load(open('/home/apurba/sph-hvi-impact-simulation/results/AlCu_dx1mm.pkl', 'rb'))
...
def make_case_figure(res, exp_crater, exp_aspect, crater_t, aspect_t,
                     snap_times, fname):
    """Split into 3 separate readable figures per reviewer feedback (Barghouty)."""
    set_rc(2.0)
    ...
    # ── Sub-figure A: all 5 particle snapshots in one row ──────────────────
    fig, axes = plt.subplots(1, 5, figsize=(30, 7),
                             gridspec_kw=dict(left=0.04, right=0.98,
    ...
    for col, si in enumerate(snap_idx):
        ...
        for mv, c in [(0, PAL_SPH), (1, PAL_PLT)]:
            ...
            ax.scatter(sn['x'][mk,0]*100, sn['x'][mk,1]*100,
                       s=8, c=c, rasterized=True, linewidths=0)
        ax.set_title(r'$t = %.1f~\mu\mathrm{s}$' % sn['t'], pad=6)
        ax.set_xlabel(r'$x$ (cm)')
        ax.set_ylabel(r'$y$ (cm)')
        ax.set_aspect('equal', 'box')
        ax.axhline(0, color='k', lw=0.6, ls=':', alpha=0.5)
    fig.legend(handles=legend_handles, loc='upper left',
               bbox_to_anchor=(0.01, 1.01), bbox_transform=fig.transFigure,
               ncol=1, framealpha=0.92, fontsize=26,
               handletextpad=0.4, borderpad=0.5, labelspacing=0.35)
    snap_fname = base + '_a_snapshots.png'
    plt.savefig(snap_fname, dpi=150, bbox_inches='tight')
    plt.close()
    print(f'Saved {snap_fname}')

    # ── Sub-figure B: crater diameter + debris aspect ratio ────────────────
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7),
                                   gridspec_kw=dict(left=0.09, right=0.97,
                                                    top=0.90, bottom=0.13,
                                                    wspace=0.38))
    # crater
    ax1.plot(t_arr, cr_arr, '-o', color=PAL_SPH, ms=7, lw=2.5, label='SPH')
    ax1.axhline(exp_crater, color=EXP_COL, ls='--', lw=2.5,
                label='Exp. %.2f cm' % exp_crater)
    ax1.axvline(crater_t, color=EVAL_COL, ls=':', lw=2)
    i_c = np.argmin(np.abs(t_arr - crater_t))
    ax1.scatter([t_arr[i_c]], [cr_arr[i_c]], s=150, c=EVAL_COL, zorder=5)
    err_c = abs(cr_arr[i_c] - exp_crater) / exp_crater * 100
    ax1.annotate('%.3f cm (%.1f%%)' % (cr_arr[i_c], err_c),
                 xy=(t_arr[i_c], cr_arr[i_c]),
                 xytext=(t_arr[i_c] + max(t_arr)*0.12, cr_arr[i_c] + 0.3),
                 arrowprops=dict(arrowstyle='->', color=EVAL_COL, lw=1.5),
                 color=EVAL_COL, fontsize=20)
    ax1.set_xlabel(r'$t$ ($\mu$s)')
    ax1.set_ylabel('Crater diameter (cm)')
    ax1.set_title('Crater diameter')
    ...
    if valid:
        tv, av = zip(*valid)
        ax2.plot(tv, av, '-o', color='#2ca02c', ms=7, lw=2.5, label='SPH')
        ax2.axhline(exp_aspect, color=EXP_COL, ls='--', lw=2.5,
                    label='Exp. %.2f' % exp_aspect)
        ax2.axvline(aspect_t, color=ASP_COL, ls=':', lw=2)
        i_a = np.argmin(np.abs(np.array(tv) - aspect_t))
        ax2.scatter([tv[i_a]], [av[i_a]], s=150, c=ASP_COL, zorder=5)
        err_a = abs(av[i_a] - exp_aspect) / exp_aspect * 100
        ax2.annotate('%.3f (%.1f%%)' % (av[i_a], err_a),
                     xy=(tv[i_a], av[i_a]),
                     xytext=(tv[i_a] + max(tv)*0.12, av[i_a] + 0.05),
    ...
    ax2.set_ylabel(r'Debris aspect ratio $l/w$')
    ax2.set_title('Debris cloud shape')
    ax2.grid(alpha=0.3)
    metrics_fname = base + '_b_metrics.png'
    plt.savefig(metrics_fname, dpi=150, bbox_inches='tight')
    plt.close()
    print(f'Saved {metrics_fname}')

    # ── Sub-figure C: energy history + speed field ─────────────────────────
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7),
                                   gridspec_kw=dict(left=0.09, right=0.97,
                                                    top=0.90, bottom=0.13,
                                                    wspace=0.38))
    ...
    st  = np.arange(len(TE))
    ax1.plot(st, TE/TE0,     'k-',  lw=2.5, label=r'Total $E$')
    ax1.plot(st, KE_arr/TE0, '--',  color='#1f77b4', lw=2, label=r'$KE$')
    ax1.plot(st, IE_arr/TE0, '--',  color='#d62728', lw=2, label=r'$IE$')
    ...
    sc  = ax2.scatter(sn_sp['x'][:,0]*100, sn_sp['x'][:,1]*100,
                      c=np.clip(spd/1e3, 0, 14), s=8,
                      cmap='hot', rasterized=True)
    cb = plt.colorbar(sc, ax=ax2, shrink=0.85)
    cb.set_label('Speed (km/s)')
    ax2.set_xlabel(r'$x$ (cm)')
    ax2.set_ylabel(r'$y$ (cm)')
    ax2.set_title(r'Speed at $t=%.0f~\mu\mathrm{s}$' % sn_sp['t'])
    ax2.set_aspect('equal', 'box')
    energy_fname = base + '_c_energy.png'
    plt.savefig(energy_fname, dpi=150, bbox_inches='tight')
    plt.close()
...
fnames_pkl = [
    '/home/apurba/sph-hvi-impact-simulation/sph_output/result_Al_Al_dx1.0mm.pkl',
    '/home/apurba/sph-hvi-impact-simulation/sph_output/result_Al_Al_dx0.75mm.pkl',
    '/home/apurba/sph-hvi-impact-simulation/sph_output/result_Al_Al_dx0.5mm.pkl',
]
